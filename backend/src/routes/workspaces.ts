...
router.get('/:id', requireAuth, loadUser, (req, res) => {
  ...
  const allDocs = db.select().from(documents)
...
router.post('/:id/documents', requireAuth, loadUser, (req, res) => {
  ...
  const maxOrder = db.select({ max: sql`coalesce(max("order"), 0)` })
    .from(workspaceDocuments)
    .where(eq(workspaceDocuments.workspaceId, workspace.id))
    .get();

  db.insert(workspaceDocuments).values({
...
router.delete('/:id/documents/:wdId', requireAuth, loadUser, (req, res) => {
  db.delete(workspaceDocuments).where(eq(workspaceDocuments.id, parseInt(req.params.wdId))).run();
  res.json({ ok: true });
});

router.delete('/:id', requireAuth, loadUser, (req, res) => {
  db.delete(workspaces).where(
    and(eq(workspaces.id, parseInt(req.params.id)), eq(workspaces.userId, req.user.id))
  ).run();
  res.json({ ok: true });
});
...
router.get('/public/:slug', (req, res) => {
  const workspace = db.select().from(workspaces)
    .where(eq(workspaces.slug, req.params.slug))
    .get();
