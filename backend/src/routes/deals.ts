...
const { deals, stakeholders, dealActivities, documents, documentViews, workspaces, workspaceDocuments } = require('../db/schema');
...
router.get('/', (req, res) => {
  try {
    ...
    const result = userDeals.map(deal => {
      const stakeholderCount = db.select({ count: sql`count(*)` })
        .from(stakeholders)
...
router.post('/', checkDealLimit, (req, res) => {
  try {
    ...
    const ws = db.select().from(workspaces)
...
router.get('/:id', (req, res) => {
  try {
    ...
    const dealStakeholders = db.select().from(stakeholders)
      .where(eq(stakeholders.dealId, deal.id))
      .all();
...
router.put('/:id', (req, res) => {
  try {
    ...
    if (stage && stage !== deal.stage) {
      ...
      addActivity(deal.id, 'stage_change', `Stage changed from ${deal.stage} to ${stage}`, { from: deal.stage, to: stage });
    }

    db.update(deals).set(updates)
...
router.post('/:id/stakeholders', (req, res) => {
  try {
    ...
    const result = db.insert(stakeholders).values({
      dealId: deal.id,
      name,
      email,
    ...
    addActivity(deal.id, 'stakeholder_added', `${name} (${role || 'influencer'}) added to deal`, { stakeholderId: result.id, email });
...
router.post('/:id/detect-stakeholders', (req, res) => {
  try {
    ...
    const existingStakeholders = db.select({ email: stakeholders.email })
      .from(stakeholders)
      .where(eq(stakeholders.dealId, deal.id))
      .all();
    ...
    for (const email of emailSet) {
      if (!existingEmails.has(email)) {
        ...
        addActivity(deal.id, 'stakeholder_added', `${name} auto-detected from document views`, { stakeholderId: result.id, email, autoDetected: true });
      }
    }
...
router.get('/:id/risk', (req, res) => {
  try {
    ...
    if (allViews.length > 0 && recentViews.length === 0) {
      signals.push({ signal: 'stalled', severity: 'high', penalty: -30, description: 'No document views in the last 7 days' });
      riskScore -= 30;
    }
    ...
    if (dealStakeholders.length > 1) {
      ...
      if (engagingCount <= 1) {
        signals.push({ signal: 'single_threaded', severity: 'medium', penalty: -20, description: `Only ${engagingCount} of ${dealStakeholders.length} stakeholders have engaged` });
        riskScore -= 20;
      }
    }
    ...
    if (champion) {
      ...
      const championOldViews = allViews.filter(v =>
      ...
      if (oldScore > 0 && (oldScore - champion.score) > 15) {
        signals.push({ signal: 'champion_weak', severity: 'high', penalty: -25, description: `Champion engagement dropped from ${oldScore} to ${champion.score}` });
        riskScore -= 25;
      }
    }
    ...
    if (deal.stage === 'negotiation') {
      ...
      if (legalStakeholder && legalStakeholder.viewCount === 0) {
        signals.push({ signal: 'legal_delay', severity: 'medium', penalty: -15, description: 'Legal stakeholder has not viewed any documents' });
        riskScore -= 15;
      }
    }
    ...
    for (const s of stakeholderScores) {
      if (s.viewCount === 1 && s.lastView && s.lastView < fiveDaysAgo) {
        if (ghostPenalty < 20) {
          signals.push({ signal: 'ghost_risk', severity: 'low', penalty: -10, description: `${s.name} viewed once but went silent` });
          ghostPenalty += 10;
          riskScore -= 10;
        }
      }
    ...
    db.update(deals).set({ healthStatus, updatedAt: new Date().toISOString() })
      .where(eq(deals.id, deal.id))
      .run();
...
router.get('/:id/brief', (req, res) => {
  try {
    ...
    if (champion) {
      const fourteenDaysAgo = new Date(Date.now() - 14 * 24 * 60 * 60 * 1000).toISOString();
      const oldViews = allViews.filter(v => v.viewerEmail && v.viewerEmail.toLowerCase() === champion.email.toLowerCase() && v.viewedAt < fourteenDaysAgo);
