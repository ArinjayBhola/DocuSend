...
router.post('/webhook', express.raw({ type: 'application/json' }), (req, res) => {
  ...
  if (event === 'subscription.activated' || event === 'subscription.charged') {
    ...
    const user = db.select().from(users)
      .where(eq(users.razorpayCustomerId, subData.customer_id))
      .get();
  ...
  if (event === 'subscription.cancelled' || event === 'subscription.completed') {
    ...
    if (subData) {
      const user = db.select().from(users)
        .where(eq(users.razorpaySubscriptionId, subData.id))
        .get();

      if (user) {
        db.update(users).set({ plan: 'free', razorpaySubscriptionId: null })
          .where(eq(users.id, user.id)).run();

        db.update(subscriptions).set({ status: 'cancelled' })
          .where(eq(subscriptions.razorpaySubscriptionId, subData.id)).run();
      }
    }
  }
