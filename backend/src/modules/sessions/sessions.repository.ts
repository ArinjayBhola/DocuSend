...
import { Session, NewSession, Participant, NewParticipant, NewSessionMessage, NewAnnotation } from './sessions.types.js';
...
export class SessionsRepository {
  ...
  async findOwnedSessions(userId: number) {
    return db
      .select({
        id: sessions.id,
  ...
  async findJoinedSessions(userId: number) {
    return db
      .select({
        id: sessions.id,
        title: sessions.title,
  ...
  async findSessionDetail(id: number) {
    return db
      .select({
        id: sessions.id,
  ...
  async getParticipantCount(sessionId: number) {
    const result = db
  ...
  async findParticipant(sessionId: number, userId: number) {
    return db
      .select()
      .from(sessionParticipants)
  ...
  async createAnnotation(data: NewAnnotation) {
    return db.insert(annotations).values(data).returning().get();
  }

  async updateAnnotation(id: number, data: Partial<NewAnnotation>) {
    db.update(annotations).set(data).where(eq(annotations.id, id)).run();
  }

  async deleteAnnotation(id: number, sessionId: number) {
  ...
  async createMessage(data: NewSessionMessage) {
    return db.insert(sessionMessages).values(data).returning().get();
  }

  async countSessionsByUserId(userId: number) {
    const result = db
      .select({ count: sql`count(*)` })
      .from(sessions)
      .where(eq(sessions.userId, userId))
