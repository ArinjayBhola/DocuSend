...
export class BillingController {
  ...
  getBillingInfo = async (req: Request, res: Response) => {
    if (!req.user) {
  ...
  subscribe = async (req: Request, res: Response) => {
    ...
    const result = await this.service.subscribe(
      ...
      req.user.name,
      req.user.email,
      req.user.razorpayCustomerId || null,
      plan
    );
  ...
  success = async (req: Request, res: Response) => {
    ...
    await this.service.success(req.user.id, razorpay_subscription_id, (plan as any) || 'pro');
    res.json({ ok: true });
  };
}
