export class DealAnalyticsService {
    computeEngagementScore(views) {
        ...
        for (const v of views) {
            ...
            if (v.viewedAt > lastVisit)
                lastVisit = v.viewedAt;
        }
    ...
    async assessRisk(deal, stakeholders, allViews) {
        let riskScore = 100;
        const signals = [];
        ...
        if (champion) {
            ...
            const championOldViews = allViews.filter((v) => v.viewerEmail && v.viewerEmail.toLowerCase() === champion.email.toLowerCase() && v.viewedAt < fourteenDaysAgo);
    ...
    computeIntentGraph(stakeholders, wsDocs, views) {
        ...
        for (const s of stakeholders) {
            ...
            for (const v of myViews) {
                if (!byDoc[v.documentId])
                    byDoc[v.documentId] = { totalTime: 0, maxCompletion: 0, views: 0 };
    ...
    computeActions(deal, stakeholders, allViews) {
        const actions = [];
