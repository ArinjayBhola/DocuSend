...
export class LeadsRepository {
    async findLeadsByUser(userId) {
        return db
            ...
            .where(and(eq(documents.userId, userId), isNotNull(documentViews.viewerEmail), sql `${documentViews.viewerEmail} != ''`))
            .groupBy(documentViews.viewerEmail, documents.id)
            .orderBy(desc(sql `max(${documentViews.viewedAt})`))
            .all(); // Cast due to complex SQL expressions
    }
}
